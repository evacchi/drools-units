...
import org.kie.api.runtime.rule.RuleUnit.Identity;

import static org.drools.core.util.ClassUtils.isAssignable;

public class LegacyRuleUnitFactory {

    private Map<Identity, RuleUnit> units = new HashMap<>();
    private Map<String, Object> variables = new HashMap<>();

    public LegacyRuleUnitFactory bindVariable( String name, Object dataSource ) {
    ...
    public <T extends RuleUnit> T getOrCreateRuleUnit( InternalRuleUnitExecutor executor, Class<T> ruleUnitClass ) {
        return (T)units.computeIfAbsent(new org.kie.api.runtime.rule.RuleUnit.Identity(ruleUnitClass), id -> createRuleUnit(executor, ruleUnitClass ) );
    }
